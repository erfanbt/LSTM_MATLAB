function [y] = sigmoid_pwl_fp(x,WL,FL,seg)
...
step=(2^integ)/seg;
for k=1:seg
    a(k)=(sigmoid(-(2^(integ-1))+step*k)-sigmoid(-(2^(integ-1))+step*(k-1)))/step;
    b(k)=sigmoid(-(2^(integ-1))+step*k)-a(k)*(-(2^(integ-1))+step*k);
end
%
% sample = -2:.0001:2; 
% v = sigmoid(sample);
